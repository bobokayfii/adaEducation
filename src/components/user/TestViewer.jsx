...
function TestViewer({ modulesData: customModulesData, moduleId, lessonIndex, lessonTitle, onCompleteTest, onBack, moduleTests }) {
  ...
  if (showResults) {
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-default border border-gray-200 p-4 sm:p-6 lg:p-8 text-center">
          <div
            className={`inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-full mb-4 sm:mb-6 ${
              passed ? 'bg-secondary/10 border-2 border-secondary' : 'bg-accent/10 border-2 border-accent'
            }`}
          >
            {passed ? (
              <Award className="w-10 h-10 sm:w-12 sm:h-12 text-secondary" />
            ) : (
              <CheckCircle className="w-10 h-10 sm:w-12 sm:h-12 text-accent" />
            )}
          </div>

          <h2 className="text-2xl sm:text-3xl font-bold gradient-text mb-2 px-4">
            {passed ? 'Поздравляем!' : 'Тест завершен'}
          </h2>

          <div className="text-4xl sm:text-5xl lg:text-6xl font-bold gradient-text my-4 sm:my-6">{score}%</div>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2 sm:gap-3">
                    {isCorrect ? (
                      <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-secondary mt-0.5 sm:mt-1 flex-shrink-0" />
                    ) : (
                      <XCircle className="w-4 h-4 sm:w-5 sm:h-5 text-danger mt-0.5 sm:mt-1 flex-shrink-0" />
                    )}
          ...
          <button
            ...
            className="bg-gradient-primary hover:shadow-hover text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg text-sm sm:text-base font-semibold transition-all transform hover:-translate-y-0.5 active:scale-95"
          >
            Вернуться к курсам
          </button>
        </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-default border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center pt-4 sm:pt-6 border-t border-gray-200 gap-3">
            <button
            ...
            <button
              onClick={handleNext}
              disabled={!isAnswered}
              className="bg-gradient-primary hover:shadow-hover text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg text-sm sm:text-base font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:-translate-y-0.5 active:scale-95"
            >
